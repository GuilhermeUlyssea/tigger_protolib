...
def process_allindividual_metrics(path)->list:
    ...
    for folder in range(len(folders)):
        ...
        for file in range(len(files)):
            with open(rf'{sub_path}\{files[file]}','r',encoding='utf-8') as f:
                ...
                mse = [float(mse[i]) for i in range(5)]
                mae = [float(mae[i]) for i in range(5)]
                r2 = [float(r2[i]) for i in range(5)]
                ...
                results_dict['MAE'].append(mae[3])
                results_dict['MAE'].append(mae[4])

                results_dict['MSE'].append(mse[0])
                results_dict['MSE'].append(mse[1])
                results_dict['MSE'].append(mse[2])
                results_dict['MSE'].append(mse[3])
                results_dict['MSE'].append(mse[4])

                results_dict['R2'].append(r2[0])
                results_dict['R2'].append(r2[1])
                results_dict['R2'].append(r2[2])        
                results_dict['R2'].append(r2[3])        
                results_dict['R2'].append(r2[4])        
...
def plot_total_rows_per_dataset(data_dict, title='Total de Linhas por Dataset',y_scale=10000):
    """
    ...
    palette = sns.color_palette("tab10", n_colors=len(dataset_names))
...
def dunn_testIII(df_mae, df_mse, df_r2, remove_outliers=True):
    ...
    for df_metric in [df_mae, df_mse, df_r2]:
        ...
        if remove_outliers:
            ...
            df_clean = (
                ...
                .apply(
                    lambda g: g.loc[
                        g[metric].between(
                            g[metric].quantile(0.25)
                            - 1.5 * (g[metric].quantile(0.75) - g[metric].quantile(0.25)),
                            g[metric].quantile(0.75)
                            + 1.5 * (g[metric].quantile(0.75) - g[metric].quantile(0.25)),
                        )
                    ]
                )
        ...
        # Kruskal-Wallis
        grupos = [
            df_clean[df_clean["grupo"] == g]["valor"]
            for g in df_clean["grupo"].unique()
        ]
        ...
        if p < 0.05:
            ...
            print("→ H₀ rejeitada: há diferença significativa")

            modelos = list(df_clean["grupo"].unique())

            print("\nTeste de Dunn (comparações par-a-par):")
...
def process_individual_metrics(path)->list:
    ...
    for folder in range(len(folders)):
        ...
        sub_path = rf'{path}\{folders[folder]}'
        files = [f for f in os.listdir(sub_path) if f.startswith('ind_metric')]
...
def process_individual_metrics_update(path)->list:
    ...
    files = [file for file in os.listdir(path) if file.startswith('ind_metric')]
    for file in files:
            with open(rf'{path}\{file}','r',encoding='utf-8') as f:
                ...
                results_dict['output'].append('YS')
                results_dict['output'].append('TS')
                results_dict['output'].append('TEL')
                results_dict['output'].append('r')
                results_dict['output'].append('n')


                for i in range(5): results_dict['split'].append(f'split-{files.index(file)}')
                for i in range(5): results_dict['MAE'].append(mae[i])
                for i in range(5): results_dict['MSE'].append(mse[i])
                for i in range(5): results_dict['R2'].append(r2[i])
...
def group_boxplot4(list_df, model_names, metric, y_min, y_max, y_step,
                  selected_outputs=None, remove_outliers=True):
    ...
    if do_dunn:
        # ---- calcular e plotar Dunn p-values ----
        ...
        for i, output in enumerate(xticks):

            df_out = df_all[df_all['output'] == output]
            ...
            pvalue = compute_dunn_pvalue(df_dunn, model_a, model_b)

            # posição do texto
            y_pos = df_out[metric].max() + (y_max - y_min) * (0.04) #0.03
    ...
    plt.legend(title='Model',title_fontsize=14,fontsize=14,loc='upper left')
    plt.tight_layout()
    plt.show()

def group_boxplot4m(list_df, model_names, metric, y_min, y_max, y_step,
                  selected_outputs=None, remove_outliers=True):
    ...
    # Tradução dos nomes dos outputs
    # translation_dict = {
    #     'LE': 'TS',
    #     'LR': 'YS',
    #     'AL': 'EL',
    ...
    for df, name in zip(list_df, model_names):
        ...
        # aplica tradução
        #temp['output'] = temp['output'].replace(translation_dict)
    ...
    if selected_outputs is not None:
        ...
        # traduz selected_outputs também
        # selected_outputs = [
        #     translation_dict.get(out, out)
    ...
    if do_dunn:
        ...
        for i, output in enumerate(xticks):
            ...
            # posição do texto
            y_pos = df_out[metric].max() + (y_max - y_min) * 0.04
